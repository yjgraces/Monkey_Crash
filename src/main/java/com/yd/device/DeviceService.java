...
public class DeviceService implements IDevice {
    ...
    public List<DeviceEntry> getAllDevice() {
        List<DeviceEntry> deviceInfoList = new ArrayList<DeviceEntry>();
        ...
        for (int j = 0; j < devicesList.size(); j++) {
            ...
            String model = null;
            String brand = null;
            String s = devicesList.get(j);
            String cmd = "adb -s " + s + " shell cat /system/build.prop";
    ...
    public boolean installApp(DeviceEntry deviceEntry, String Path) {
        ...
        if (!(Path.isEmpty()) && Path.endsWith(".apk")) {
            ...
            flag = response.contains("Success");
        } else {
...
//        DeviceService d = new DeviceService(commendManager);
...
//        commendManager.close();
		 ...
		 */
         ...
         String deviceName="T600";
         try {
         String
         blacklistpath="E:/Git/Monkey_Crash_Test/src/main/resources/config/"+deviceName+"_blacklist.txt";
